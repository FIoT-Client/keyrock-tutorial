...
Para que seja possível executar qualquer uma das aplicações é necessário fazer o clone deste repositório e ter realizado a instalação, configuração e execução da infraestrutura, dispońível no início deste tutorial. Antes de começar é necessário escolher qual aplicação você deseja utilizar, assim você poderá escolher entre as seguintes:

Aplicações
----------

Nessa sessão iremos apresentar as aplicações disponíveis neste tutorial para que você possa decidir qual utilizar.
